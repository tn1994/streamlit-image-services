        ...
        try:
            ...
            with tab1:
                if st.button('Select All'):
                    with st.spinner('Wait for it...'):
                        res = notion_pinterest_service.show_database(
            ...
            with tab2:
                with st.form(key='notion_pinterest_insert_service_form'):
                    ...
                    tag: str = st.selectbox(
                        label='Select Tag',
                        options=notion_pinterest_service.tag_list)
                    hash: str = make_hashes(password=link_id)
                    submitted = st.form_submit_button(label='CREATE')

                if 0 != len(link_id) and 0 != len(
                        tag) and 0 != len(hash) and submitted:
                    with st.spinner('Wait for it...'):
                        notion_pinterest_service.insert_item(
                            database_id=self.database_id, link_id=link_id, tag=tag, hash=hash)
                    st.json(notion_pinterest_service.result_dict)
            ...
            with tab3:
                ...
                if 0 != len(tag) and submitted:
                    with st.spinner('Wait for it...'):
                        res = notion_pinterest_service.query(
                            query=tag, database_id=self.database_id)
                    st.table(res)
                    st.json(res)
